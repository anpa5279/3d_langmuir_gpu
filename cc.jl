...
@inline K_b(T, S) = exp((-8966.90 - 2890.53 * sqrt(S) - 77.942 * S + 1.728 * S^1.5 - 
            ...
            + 0.053105 * sqrt(S) * (T + 273.15)) * 1.0e6

#Zeebe and Wolf Gladrow 2001
@inline alpha1(T) = exp(1246.98 - 6.19e4 / (T + 273.15) - 183.0 * log(T + 273.15)) / 1e6  # kg/umol/s
@inline beta1(alpha1, K1) = alpha1/ K1 # 1/s
@inline alpha2(A1, E1, T) = A1 * exp(-E1 / (R * (T + 273.15))) / 1e6 # kg/umol/s
@inline beta2(alpha2, Kw, K1) = alpha2 * Kw/ K1# 1/s
@inline beta3(alpha3, K2) = alpha3 * K2 # 1/s
@inline beta4(alpha4, Kw, K2) = alpha4 * Kw/ K2 # 1/s
@inline beta5(alpha5, Kw) = alpha5 / Kw * 1e6 # umol/kg/s
@inline alpha6(A7, E8, T) = A7 * exp(-E8 / (R * (T + 273.15))) / 1e6 # kg/umol/s
@inline beta6(alpha6, Kw, Kb) =  alpha6* Kw/ Kb # 1/s
@inline alpha7(A8, E8, T) = A8 * exp(-E8 / (R * (T + 273.15))) / 1e6 # kg/umol/s
@inline beta7(alpha7, K2, Kb) = alpha7* K2/ Kb / 1e6 # kg/umol/s
