...
#variations
dxs = [2.0, 1.0, 0.5]
advection_schemes = [WENO(order = 3), WENO(order = 5), WENO(order = 9)]
names = ["WENO3", "WENO5", "WENO9"]
for (name, scheme) in zip(names, advection_schemes)
    for dx in dxs
        Nx = Ny = Int(Lx/dx)
        Nz = Int(Lz/dx)
        dir = "localoutputs/scheme-tests/longer/$(name)/dx$(dx)/"
        @show dir
        @show "Running simulation with dx=$(dx)(Nx = $(Nx), Ny = $(Ny), Nz = $(Nz)) and scheme=$(scheme)"
        ...
        # defining simulation
        simulation = Simulation(model, Δt=min_step, stop_time = 4.0hours, minimum_relative_step = 0.01) 
        ## progress function
