...
damp_rates = [1/240, 1/120, 1/60, 1/30, ] # [1/s], relaxation rate for all masks
width_opts = [5, 20, ]
T_target(x, y, z, t) = Tᵢ(0, y, z)
for damp_rate in damp_rates
    for width_opt in width_opts
        # gaussian mask
        gaus_mask_x1 = GaussianMask{:x}(center = Lx/2, width = width_opt)
        gaus_mask_x2 = GaussianMask{:x}(center = -Lx/2, width = width_opt)
        ...
        gaus_mask_y1 = GaussianMask{:y}(center = Ly/2, width = width_opt)
        gaus_mask_y2 = GaussianMask{:y}(center = -Ly/2, width = width_opt)
        ...
        # linear mask
        linear_mask_x1 = PiecewiseLinearMask{:x}(center = Lx/2, width = width_opt)
        linear_mask_x2 = PiecewiseLinearMask{:x}(center = -Lx/2, width = width_opt)
        ...
        linear_mask_y1 = PiecewiseLinearMask{:y}(center = Ly/2, width = width_opt)
        ...
        linear_mask_y2 = PiecewiseLinearMask{:y}(center = -Ly/2, width = width_opt)
        ...
        for (i, mask) in enumerate([gaus, linear]) #[gaus, linear, nothing]
            ...
            rel_path = "localoutputs/sponge testing/$(mask_str[i])/width = $(width_opt), rate = $(damp_rate)/"
