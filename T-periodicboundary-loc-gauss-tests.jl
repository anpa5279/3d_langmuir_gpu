...
#variations
dxs = [2.0, 1.0, 0.5]
advection_schemes = [WENO(order = 3), WENO(order = 5)]
names = ["WENO3", "WENO5"]
for (name, scheme) in zip(names, advection_schemes)
    for dx in dxs
        Nx = Ny = Int(Lx/dx)
        Nz = Int(Lz/dx)
        dir = "localoutputs/scaled S and w with half pi to match both gauss/$(name)/dx$(dx)/"
        @show dir
        println("Running simulation with dx=$(dx)(Nx = $(Nx), Ny = $(Ny), Nz = $(Nz)) and scheme=$(scheme)")
        ...
        factor = gaus_area_cont/gaus_area_discr
        @show factor
        mass_factor = pi/2*((2*rp)^2)/gaus_area_discr # matching mass flux into the domain
        @show mass_factor
        @inline w_surface(x, y, t)=wp*factor*mass_factor*exp(-((x^2 + y^2)/(rp^2))) 

        w_bottom = sum(w_surface.(repeat(grid.xᶜᵃᵃ[1:Nx], 1, Ny), transpose(repeat(grid.yᵃᶜᵃ[1:Ny], 1, Nx)), 0.0))/(Nx*Ny)
