...
@inline function ∂ⱼ_τ₁ⱼ(i, j, k, grid, clock, model_fields)
    ...
    w = model_fields.w
    ν = model_fields.νₑ
    return @inbounds -1 / Vᶠᶜᶜ(i, j, k, grid) * (δxᶠᵃᵃ(i, j, k, grid, Ax_qᶜᶜᶜ, viscous_flux_ux, ν, u) +
...
@inline function ∇_dot_qᶜ(i, j, k, grid, clock, model_fields)
    ...
    return @inbounds -1 / Vᶜᶜᶜ(i, j, k, grid) * (δxᶜᵃᵃ(i, j, k, grid, Ax_qᶠᶜᶜ, diffusive_flux_x, ν, scalar) +
