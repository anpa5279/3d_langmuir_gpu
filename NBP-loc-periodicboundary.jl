...
@inline function CaCO3_t(x, y, t) 
    if (t <= 6hours)
        σ = 10.0 # m
        c0 = -u_f*mass/(molar_calcite*(Lx/Nx)*(Ly/Ny)*(Lz/Nz)) # mol/m3
...
Tᵢ(x, y, z) = z > - initial_mixed_layer_depth ? T0 : 
                T0 + dTdz * (z + initial_mixed_layer_depth)+dTdz * model.grid.Lz * 1e-6 * r(x, y, z)
...
set!(model, u=uᵢ, v=vᵢ, T=Tᵢ, CaCO3=0.0)#CaCO3ᵢ)

# defining simulation
simulation = Simulation(model, Δt=30, stop_time = 2.0*24hours) 
@show simulation
# physics callbacks
function make_positive!(model)
    C = model.tracers.CaCO3
    @inbounds for i in eachindex(C)
        if C[i] < 0.0
            model.tracers.CaCO3[i] = 0.0
        end
    end
    return nothing 
end

simulation.callbacks[:post_step] = Callback(make_positive!, IterationInterval(1), callsite=UpdateStateCallsite())
