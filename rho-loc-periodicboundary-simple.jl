...
for (i, res) in enumerate(["uniform", "doubled vertical resolution"])
    for N in (16, 32, 64, 128)
        println("Running simulation with $res and N = $N")
        ## referring to files with desiraed functions
        grid = RectilinearGrid(; size=(N, N, i*N), extent=(Lx, Ly, Lz))
        @show grid
        ## defining model
        ...
        path = "localoutputs/b tracer for NBP/flux b tracer $res N = $N/"
